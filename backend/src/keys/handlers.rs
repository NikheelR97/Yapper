...
use super::service::{
    parse_device_ids_filter, reject_trivial_x25519_key, resolve_signal_device_id,
    verify_backup_restore_signature, verify_signed_prekey_signature, BACKUP_RETRIEVE_LIMITER,
};
use super::types::*;
...
/// GET /api/v2/keys/one-time-prekey-count — returns the number of unconsumed OPKs
...
/// GET /api/v2/keys/:user_id — fetch a user's public key bundle for X3DH.
///
...
/// PUT /api/v2/keys/backup — store an opaque PIN-encrypted key backup blob.
///
...
pub async fn upload_identity_key_v2(
    auth: AuthDevice,
    State(state): State<AppState>,
...
pub async fn upload_signed_prekey_v2(
    auth: AuthDevice,
    State(state): State<AppState>,
...
pub async fn upload_one_time_prekeys_v2(
    auth: AuthDevice,
    State(state): State<AppState>,
...
pub async fn get_key_bundles_v2(
    auth: AuthDevice,
    State(state): State<AppState>,
    Path(user_id): Path<Uuid>,
...
pub async fn restore_backup_v2(
    auth: AuthDevice,
    State(state): State<AppState>,
