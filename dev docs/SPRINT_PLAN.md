...
**Shipped follow-up (2026-03-09):** the production auth surface now runs on device-aware `v2` auth while `v1` remains as a migration compatibility layer. Current production login/register send device bootstrap metadata (`installation_id`, `platform`, `label`), receive back `device` metadata alongside `access_token`/`csrf_token`, restore via `POST /api/v2/auth/refresh`, and complete OAuth on the frontend with `POST /api/v2/auth/attach-device`. New installs can enter `pending_trust` until an existing trusted device approves them or an encrypted backup is restored.
...
| App loading screen | `lib/components/AppLoadingScreen.svelte` | Shown while auth restore, device approval, vault unlock, and Signal bootstrap complete; sphere pulse, indeterminate progress bar |
...
### Auth v2 Device Flow (Shipped 2026-03-09)

- `POST /api/v2/auth/login` and `POST /api/v2/auth/register` are the primary production entrypoints for frontend auth.
- `POST /api/v2/auth/refresh` restores the session for the active device and returns the current `device` summary to the auth store.
- `POST /api/v2/auth/attach-device` is used after OAuth callback and during session hydration to bind the current installation to the signed-in account.
- Device trust state is now part of login UX:
  - `trusted` devices continue into the app shell and can approve other devices
  - `pending_trust` devices stop at the approval / recovery gate until trusted
  - `revoked` devices are ejected, local vault is wiped, and the user returns to login
- Same-device logout/login is expected to preserve the local E2EE vault; only explicit device revocation / forget-device clears local key material.
...
> **S11 Note (2026-03-09):** Full SAST pen-test audit completed — 63 vulnerabilities found (3 CRITICAL, 11 HIGH, 22 MEDIUM, 18 LOW, 9 INFO). Security hardening sprint (S12) added. **E2E testing and launch BLOCKED until S12 Phase 1 (CRITICAL + HIGH fixes) is complete.** Full report: `dev docs/VULNERABILITY_REPORT.md`.

---

## S12 — Security Hardening (Weeks 25–26)

**Goal:** Fix all CRITICAL and HIGH vulnerabilities identified in the full SAST penetration test before any public beta. Address MEDIUM findings as time allows.
