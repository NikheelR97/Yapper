<script lang="ts">
	...
	import { authStore, clearAuth, setAuth, type AuthDevice } from "$stores/auth.js";
	import { ApiError, api } from "$api/client.js";
	import type { User } from "$stores/auth.js";
	import { toast } from "$stores/toast.js";
	import { onWsMessage, wsConnect, wsDisconnect, wsStore } from "$stores/ws.js";
	import { setupKeys, replenishPreKeysIfNeeded, restoreKeys } from "$signal/index.js";
	import {
	...
	function setPendingRestoreDevices(
		...
		restoreDevices = devices
			.filter((device) => device.id !== currentDeviceId && device.trust_state === "trusted")
	...
	function splitSyncSnapshot(snapshot: Uint8Array): Uint8Array[] {
		...
		for (let offset = 0; offset < snapshot.length; offset += DEVICE_SYNC_CHUNK_SIZE) {
	...
	function dedupeDeviceSyncEvents(events: DeviceSyncEvent[]): DeviceSyncEvent[] {
	...
	async function syncTrustedSnapshotToDevice(targetDeviceId: string): Promise<boolean> {
		...
		for (let index = 0; index < chunks.length; index += 1) {
			const encryptedChunk = await encryptDeviceSyncChunk(chunks[index], syncPublicKey);
	...
	async function processPendingSyncEvents(
		...
		const chunkEvents = events.filter((event) => event.event_type === "device_sync_chunk");
		for (const event of chunkEvents) {
			const transferId =
				typeof event.payload.transfer_id === "string" ? event.payload.transfer_id : null;
			const ciphertext =
				typeof event.payload.ciphertext === "string" ? event.payload.ciphertext : null;
			const ekPublic =
				typeof event.payload.ek_public === "string" ? event.payload.ek_public : null;
			const index =
				typeof event.payload.index === "number" ? event.payload.index : Number.NaN;
			const total =
				typeof event.payload.total === "number" ? event.payload.total : Number.NaN;
			if (!transferId || !ciphertext || !ekPublic || !Number.isInteger(index) || !Number.isInteger(total)) {
				continue;
			}

			try {
				const plaintext = await decryptDeviceSyncChunk(deviceId, ciphertext, ekPublic);
				const transfer =
					incomingSyncTransfers.get(transferId) ??
					{ total, chunks: Array.from({ length: total }, () => "") };
		...
		for (const event of events) {
			...
			if (!transfer || transfer.total !== total || transfer.chunks.some((chunk) => !chunk)) {
				continue;
			}
	...
	async function handleTrustedRealtimeSyncEvent(event: DeviceSyncEvent): Promise<void> {
		...
		if (event.event_type === "device_revoked") {
			const revokedDeviceId =
				typeof event.payload.device_id === "string" ? event.payload.device_id : null;
			if (!revokedDeviceId) {
				return;
			}
	...
	async function handleRealtimeDeviceSyncEvent(event: DeviceSyncEvent): Promise<void> {
		...
		const { trustApproved, importedSnapshot } = await processPendingSyncEvents([nextEvent], deviceId);
	...
	async function refreshPendingDeviceStatus() {
		...
		try {
			...
			if (state.device) {
				...
				const syncEvents = dedupeDeviceSyncEvents(await fetchDeviceSyncEvents());
				const { trustApproved, importedSnapshot } = await processPendingSyncEvents(
					syncEvents,
					state.device.id,
				);
				if (trustApproved || importedSnapshot) {
					if (await maybeActivateTrustedDevice(state.device.id, importedSnapshot)) {
						return;
					}
				}
			}
			...
			const current = devices.find((device) => device.id === nextState.device?.id);
	...
	async function restorePendingDeviceFromBackup(
		...
		try {
			...
			if (await maybeActivateTrustedDevice(deviceId, true)) {
				toast.success("Encrypted backup restored. This device replaced the source device.");
				return;
			}
