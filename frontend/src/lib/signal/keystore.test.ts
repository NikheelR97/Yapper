...
describe('keystore legacy migration', () => {
	...
	it('migrates legacy receiver keys and emoji cache into the scoped store', async () => {
		...
		await legacyDb.put('emojis', [{ id: 'emoji-1', name: 'party', imageUrl: 'https://cdn.test/party.png' }], 'server-1');
		legacyDb.close();

		await expect(configureSignalStore('user-1', 'device-1')).resolves.toBeUndefined();
		...
		expect(await loadSenderKey('channel-1')).toMatchObject({ channelId: 'channel-1', iteration: 2 });
		expect(await loadReceiverKey('channel-1', 'peer-1', 'legacy')).toMatchObject({
			channelId: 'channel-1',
			senderId: 'peer-1',
			iteration: 1,
		});
