...
const DIST_SIG_INFO = new TextEncoder().encode("YapperSenderKeyDistSig_v2");
...
function assertValidX25519PublicKey(
...
function senderKeyDistSignatureInput(payload: {
  channelId: string;
  senderUserId: string;
  ...
  return concat(
    DIST_SIG_INFO,
    encodeUtf8(payload.channelId),
    new Uint8Array([0]),
    encodeUtf8(payload.senderUserId),
    new Uint8Array([0]),
    encodeUtf8(payload.senderDeviceId),
    new Uint8Array([0]),
    iterationBytes(payload.iteration),
    b64ToBytes(payload.chainKey),
    b64ToBytes(payload.signingPubKey),
  );
}

export function signSenderKeyDistPayload(
  payload: Omit<SenderKeyDistPayload, "identitySignature">,
  identity: IdentityKeyPair,
): SenderKeyDistPayload {
  return {
    ...payload,
    identitySignature: bytesToB64(
      ed25519.sign(
        senderKeyDistSignatureInput(payload),
...
export function verifySenderKeyDistPayload(
  payload: SenderKeyDistPayload,
  identitySigPubKey: Uint8Array,
  expectedSenderUserId?: string,
  expectedSenderDeviceId?: string,
): void {
  if (expectedSenderUserId && payload.senderUserId !== expectedSenderUserId) {
    throw new Error("Sender key distribution user mismatch");
  }
  if (
    expectedSenderDeviceId &&
    payload.senderDeviceId !== expectedSenderDeviceId
  ) {
    throw new Error("Sender key distribution device mismatch");
  }
  if (!payload.identitySignature) {
    throw new Error("Sender key distribution is missing an identity signature");
  }
  const ok = ed25519.verify(
    b64ToBytes(payload.identitySignature),
    senderKeyDistSignatureInput({
  ...
  if (!ok) {
    throw new Error("Sender key distribution identity signature is invalid");
  }
}
...
export async function decryptWithSenderKey(
  ...
  if (iteration < record.iteration) {
    if (!allowHistorical) {
      throw new Error(
        `[SenderKey] Message iteration ${iteration} already consumed (at ${record.iteration})`,
      );
    }
    ...
    if (!initialChainKey || iteration < initialIteration) {
      throw new Error(
        `[SenderKey] No historical key material for iteration ${iteration}`,
      );
    }
    ...
    const derived = deriveForIteration(
      initialChainKey,
      initialIteration,
      iteration,
    );
    ...
    if (iteration - record.iteration > MAX_SKIP) {
      throw new Error(
        `[SenderKey] Message iteration ${iteration} too far ahead (at ${record.iteration})`,
      );
    }
  ...
  const validSig = ed25519.verify(
    b64ToBytes(encrypted.signature),
    sigInput,
    record.signingPubKey,
  );
  if (!validSig) {
 ...
 */
export function unpackChannelMessage(
  wireCt: string,
  iteration: number,
): EncryptedChannelMessage {
