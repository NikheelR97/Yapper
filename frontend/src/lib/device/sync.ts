...
function mergeById<T extends { id: string }>(primary: T[], secondary: T[]): T[] {
	...
	return [...records.values()].sort((a, b) => {
		const aCreatedAt = 'created_at' in a && typeof a.created_at === 'string' ? a.created_at : '';
		const bCreatedAt = 'created_at' in b && typeof b.created_at === 'string' ? b.created_at : '';
...
function saveStoredSyncRequest(deviceId: string, request: StoredSyncRequest): void {
	if (typeof window === 'undefined') {
		return;
	}

	window.localStorage.setItem(syncRequestKey(deviceId), JSON.stringify(request));
}
...
export function ensurePendingDeviceSyncRequest(deviceId: string): { publicKey: string } {
...
export async function encryptDeviceSyncChunk(
	...
	const inputKeyMaterial = concat(sharedSecret, ephemeralPublicKey, recipientPublicKeyBytes);
	const encryptionKey = hkdf(sha256, inputKeyMaterial, undefined, SYNC_INFO, 32);

	const iv = crypto.getRandomValues(new Uint8Array(12));
	const aesKey = await crypto.subtle.importKey('raw', encryptionKey.slice(), 'AES-GCM', false, ['encrypt']);
	const ciphertext = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, aesKey, plaintext.slice());
...
export async function decryptDeviceSyncChunk(
	...
	const sharedSecret = x25519.getSharedSecret(requestPrivateKey, ephemeralPublicKey);
	const inputKeyMaterial = concat(sharedSecret, ephemeralPublicKey, myPublicKey);
	const encryptionKey = hkdf(sha256, inputKeyMaterial, undefined, SYNC_INFO, 32);

	const iv = ciphertextBytes.slice(0, 12);
	const encryptedBytes = ciphertextBytes.slice(12);
	const aesKey = await crypto.subtle.importKey('raw', encryptionKey.slice(), 'AES-GCM', false, ['decrypt']);
	const plaintext = await crypto.subtle.decrypt({ name: 'AES-GCM', iv }, aesKey, encryptedBytes);
	return new Uint8Array(plaintext);
}

async function fetchConversationHistory(): Promise<CachedDmHistoryMessage[]> {
	const conversations = await api.get<SyncConversation[]>('/api/v2/conversations').catch(() => []);
...
async function fetchChannelHistory(): Promise<CachedChannelHistoryMessage[]> {
	const servers = await api.get<SyncServer[]>('/api/v1/servers').catch(() => []);
	if (!servers.length) {
		return [];
	}
	...
	const channels = (
		await Promise.all(
			servers.map((server) =>
				api.get<SyncChannel[]>(`/api/v1/servers/${server.id}/channels`).catch(() => [])
			)
		)
	).flat();
...
async function fetchPaginatedHistory<T extends { id: string }>(basePath: string): Promise<T[]> {
	...
	while (true) {
		...
		const page = await api.get<T[]>(`${basePath}?${params.toString()}`).catch(() => []);
		if (!page.length) {
			break;
		}
...
export async function importTrustedDeviceSnapshot(serializedSnapshot: string): Promise<void> {
	const snapshot = JSON.parse(serializedSnapshot, jsonReviver) as {
		...
		channelHistory?: Parameters<typeof importSignalSnapshot>[0]['channelHistory'];
	};
